...
import static fr.osb.smartf.agent.worker.batch.patrimony.ImportPatrimonyJobConfig.IMPORT_INSTALLED_PATRIMONY_XML;

/**
 * Created by mpaltanea on 13.07.2016.
 */
@RestController
@RequestMapping("/installedPatrimony")
public class InstalledPatrimonyController {

    private static final Logger LOG = LoggerFactory.getLogger(InstalledPatrimonyController.class);

    private final LaunchJobService launchJobService;

    @Autowired
    public InstalledPatrimonyController(LaunchJobService launchJobService) {
    ...
    public Response importXmlFile(@RequestBody ImportFileParams importFileParams) {
        try {
            launchJobService.launchJob(IMPORT_INSTALLED_PATRIMONY_XML, ImportType.installedPatrimony, ImportFormat.FILE_XML, importFileParams);
        } catch (Exception e) {
